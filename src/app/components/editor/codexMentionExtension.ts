...
export interface CodexMentionItem {
  id: string;
  entryId: string;
  startIndex: number;
  endIndex: number;
...
interface CodexMentionPluginState {
  decorations: DecorationSet;
  mentions: CodexMentionItem[];
}
...
export const codexMentionPluginKey = new PluginKey("codexMentions");

export const CodexMentionExtension = Extension.create({
  name: "codexMentions",

  addProseMirrorPlugins() {
    return [
      new Plugin({
        key: codexMentionPluginKey,
        state: {
          init(): CodexMentionPluginState {
            return {
              decorations: DecorationSet.empty,
              mentions: [],
            };
          },
          apply(tr, prev, _oldState, newState): CodexMentionPluginState {
            const meta = tr.getMeta(codexMentionPluginKey) as
              | { mentions: CodexMentionItem[] }
              | undefined;
  ...
  for (const mention of mentions) {
    ...
    decorations.push(
      Decoration.inline(from, to, {
        ...
        "data-codex-mention-id": mention.id,
      })
    );
  }
