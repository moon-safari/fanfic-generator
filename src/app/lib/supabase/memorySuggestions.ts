...
type DbMemoryChangeSuggestionRow = {
  id: string;
  story_id: string;
  chapter_id: string;
...
export async function fetchMemorySuggestions(
...
): Promise<MemoryChangeSuggestion[]> {
  let query = supabase
  ...
  return ((data ?? []) as DbMemoryChangeSuggestionRow[])
    .map(mapMemoryChangeSuggestionRow)
    .filter((suggestion): suggestion is MemoryChangeSuggestion => suggestion !== null);
}

export async function insertMemoryChangeSuggestions(
  supabase: SupabaseClient,
...
export async function clearPendingMemorySuggestionsForChapter(
  supabase: SupabaseClient,
  chapterId: string
): Promise<void> {
...
export async function updateMemorySuggestionStatus(
...
): Promise<MemoryChangeSuggestion | null> {
  ...
  return mapMemoryChangeSuggestionRow(data as DbMemoryChangeSuggestionRow);
}

export function mapMemoryChangeSuggestionRow(
  row: DbMemoryChangeSuggestionRow
): MemoryChangeSuggestion | null {
  if (!isMemoryChangeType(row.change_type)) {
    return null;
  }
...
function normalizeConfidence(value: string): MemorySuggestionConfidence {
...
function normalizeStatus(value: string): MemorySuggestionStatus {
  if (
    value === "pending" ||
    value === "accepted" ||
    value === "rejected" ||
...
function isMemoryChangeType(value: string): value is MemoryChangeType {
  return (
    value === "create_entry" ||
