...
import { buildNewsletterIssueReadinessReport } from "../../../../lib/newsletterPreflight";
...
export async function GET(
  ...
  try {
    ...
    if (story.project_mode !== "newsletter") {
      return NextResponse.json(
        { error: "Issue readiness is only available for newsletter projects" },
        { status: 400 }
      );
    }
    ...
    const report = buildNewsletterIssueReadinessReport({
    ...
    console.error("Newsletter issue readiness failed:", error);
    const message =
      error instanceof Error ? error.message : "Failed to load issue readiness";
