...
export async function authenticateCodexStory(
  storyId: string,
  storySelect = "id"
):
  Promise<
...
export async function authenticateCodexSuggestion(
  suggestionId: string,
  suggestionSelect = "*"
):
  Promise<
...
export async function getCodexEntryRecord(
  supabase: SupabaseClient,
  storyId: string,
  entryId: string,
  select = "id"
...
export async function getCodexRelationshipRecord(
  supabase: SupabaseClient,
  storyId: string,
...
export async function getCodexProgressionRecord(
  supabase: SupabaseClient,
  entryId: string,
...
export async function getCodexSuggestionRecord(
  supabase: SupabaseClient,
  storyId: string,
  suggestionId: string,
  select = "id"
...
export function parseCustomFields(value: unknown): CodexCustomField[] | null {
  if (!Array.isArray(value)) return null;

  const fields: CodexCustomField[] = [];
...
export function parseSuggestedFields(
  value: unknown
): CodexSuggestedField[] | null {
  if (!Array.isArray(value)) return null;

  const fields: CodexSuggestedField[] = [];
  for (const item of value) {
    if (!item || typeof item !== "object" || Array.isArray(item)) {
      return null;
    }
...
export function parseSuggestionPayload(
  ...
  return value as unknown as CodexChangeSuggestionPayload;
}
