...
import {
  mapCodexChangeSuggestionRow,
  updateCodexSuggestionStatus,
...
export async function POST(
  ...
  try {
    ...
    const auth = await authenticateCodexSuggestion(suggestionId);

    if (isRouteError(auth)) {
      return auth.error;
    }

    const suggestion = mapCodexChangeSuggestionRow(
    ...
    switch (suggestion.changeType) {
      case "create_entry": {
        ...
        const updatedSuggestion = await updateCodexSuggestionStatus(
          auth.supabase,
          suggestionId,
          "applied",
          {
      ...
      case "update_entry_aliases": {
        ...
        const entryRecord = await getCodexEntryRecord(
          auth.supabase,
          storyId,
          payload.entryId,
          "id, aliases"
        ...
        const entry = await updateCodexEntry(auth.supabase, storyId, payload.entryId, {
      ...
      case "create_relationship": {
        ...
        const relationship = await insertCodexRelationship(auth.supabase, storyId, {
          sourceEntryId,
          targetEntryId,
      ...
      case "create_progression": {
        ...
        if (
          ...
          return NextResponse.json(
            {
              error:
                "A progression already exists for this entry and chapter. Edit it directly from the Codex entry.",
            },
            { status: 409 }
          );
        }
    ...
    console.error("Codex suggestion accept error:", err);
