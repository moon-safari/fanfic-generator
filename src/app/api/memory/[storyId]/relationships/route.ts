...
import { insertMemoryRelationship } from "../../../../lib/supabase/memory";
import {
  authenticateMemoryStory,
  isRouteError,
  isUniqueViolation,
} from "../../shared";
import type { CreateMemoryRelationshipInput } from "../../../../types/memory";
...
export async function POST(
  ...
  try {
    ...
    const input: CreateMemoryRelationshipInput = {
      sourceEntryId,
      targetEntryId,
      forwardLabel:
    ...
    const relationship = await insertMemoryRelationship(
      auth.supabase,
      storyId,
      input
    );
    ...
    console.error("Memory relationship create error:", err);
