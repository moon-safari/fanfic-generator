...
import { deleteCodexEntry, updateCodexEntry } from "../../../../../lib/supabase/codex";
import {
...
import type { UpdateCodexEntryInput } from "../../../../../types/codex";
...
export async function PUT(
  ...
  try {
    ...
    const existingEntry = await getCodexEntryRecord(auth.supabase, storyId, entryId);
    if (!existingEntry) {
      return NextResponse.json({ error: "Entry not found" }, { status: 404 });
    }

    const body = (await req.json()) as Record<string, unknown>;
    const updates: UpdateCodexEntryInput = {};
    ...
    const entry = await updateCodexEntry(auth.supabase, storyId, entryId, updates);
    if (!entry) {
    ...
    console.error("Codex entry update error:", err);
...
export async function DELETE(
  ...
  try {
    ...
    const ok = await deleteCodexEntry(auth.supabase, storyId, entryId);
    if (!ok) {
    ...
    console.error("Codex entry delete error:", err);
