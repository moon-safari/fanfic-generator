...
import { buildCodexSuggestionPrompt } from "../../../../lib/prompts/codexSuggestions";
import { fetchCodexData } from "../../../../lib/supabase/codex";
import {
  clearPendingCodexSuggestionsForChapter,
  insertCodexChangeSuggestions,
} from "../../../../lib/supabase/codexSuggestions";
import {
...
import type {
  ...
  CodexSuggestionGenerationResponse,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const auth = await authenticateCodexStory(storyId, "id, project_mode");
      ...
      return NextResponse.json(
        {
          suggestions: [],
          skipped: true,
          reason: "Codex change detection is not enabled for newsletter projects yet.",
        },
        { status: 200 }
      );
    }
    ...
    const prompt = buildCodexSuggestionPrompt(
    ...
    if (!jsonMatch) {
      return NextResponse.json(
        { error: "Failed to parse Codex suggestion JSON" },
        { status: 500 }
      );
    }

    let parsed: CodexSuggestionGenerationResponse;
    try {
      parsed = JSON.parse(jsonMatch[0]) as CodexSuggestionGenerationResponse;
    } catch {
      return NextResponse.json(
        { error: "Invalid JSON in Codex suggestion response" },
        { status: 500 }
      );
    }
    ...
    await clearPendingCodexSuggestionsForChapter(auth.supabase, chapterId);
    const suggestions = await insertCodexChangeSuggestions(auth.supabase, rows);

    return NextResponse.json({ suggestions }, { status: 200 });
  } catch (err) {
    console.error("Codex suggestion generation error:", err);
    const message =
      err instanceof Error ? err.message : "Failed to generate Codex suggestions";
...
function buildSuggestionRows(
  ...
  generated: CodexSuggestionGenerationResponse
...
function buildEntryMap(entries: CodexEntry[]): Map<string, CodexEntry> {
...
function findEntryByName(
  entryMap: Map<string, CodexEntry>,
  name: string
): CodexEntry | null {
...
function parseCreateEntries(
  value: CodexSuggestionGenerationResponse["createEntries"]
): GeneratedCreateEntrySuggestion[] {
...
function parseAliasSuggestions(
  value: CodexSuggestionGenerationResponse["updateAliases"]
): GeneratedAliasSuggestion[] {
  if (!Array.isArray(value)) return [];
...
function parseRelationshipSuggestions(
  value: CodexSuggestionGenerationResponse["createRelationships"]
): GeneratedRelationshipSuggestion[] {
  if (!Array.isArray(value)) return [];
...
function parseProgressionSuggestions(
  value: CodexSuggestionGenerationResponse["createProgressions"]
): GeneratedProgressionSuggestion[] {
  if (!Array.isArray(value)) return [];
...
function parseStaleEntrySuggestions(
  value: CodexSuggestionGenerationResponse["flagStaleEntries"]
): GeneratedStaleEntrySuggestion[] {
  if (!Array.isArray(value)) return [];
...
function normalizeConfidence(value: unknown): CodexSuggestionConfidence | undefined {
