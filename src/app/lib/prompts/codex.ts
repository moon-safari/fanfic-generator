...
export function buildCodexGenerationPrompt(
  ...
  return `You are a story analyst. Extract an initial Codex from the provided chapter text.
${fandomContext ? `\nFANDOM CONTEXT:\n${fandomContext}\n` : ""}
...
export function resolveCodexEntryAtChapter(
  entry: CodexEntry,
...
export function formatCodexForPrompt(
  entries: CodexEntry[],
  ...
  const groups = new Map<string, ResolvedCodexEntry[]>();
...
function formatEntryBlock(
  entry: ResolvedCodexEntry,
  relationships: CodexRelationship[],
  entryMap: Map<string, CodexEntry>,
  resolvedById: Map<string, ResolvedCodexEntry>
): string[] {
  const lines: string[] = [];
...
function resolveFallbackEntry(
  entryId: string,
  entryMap: Map<string, CodexEntry>
): ResolvedCodexEntry | null {
  const entry = entryMap.get(entryId);
  return entry ? resolveCodexEntryAtChapter(entry, Number.MAX_SAFE_INTEGER) : null;
}

function toFieldMap(fields: CodexCustomField[]): Record<string, string> {
...
function fromFieldMap(fieldMap: Record<string, string>): CodexCustomField[] {
