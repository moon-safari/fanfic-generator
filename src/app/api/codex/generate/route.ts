...
import { buildCodexGenerationPrompt } from "../../../lib/prompts/codex";
import {
  fetchCodexData,
  mapCodexEntryRow,
...
import type {
  CodexGenerateEntryInput,
  CodexGenerateRelationshipInput,
} from "../../../types/codex";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const auth = await authenticateCodexStory(storyId, "id, fandom, project_mode");
      ...
      return NextResponse.json(
        {
          ...
          relationships: [],
          skipped: true,
          reason: "Codex auto-generation is not enabled for newsletter mode yet.",
        },
        { status: 200 }
      );
    }
    ...
    if (!jsonMatch) {
      return NextResponse.json(
        { error: "Failed to parse codex JSON" },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: "Invalid JSON in codex response" },
        { status: 500 }
      );
    }
    ...
    const existingCodex = await fetchCodexData(auth.supabase, storyId);

    const existingNames = new Set(
      existingCodex.entries.map((entry) => normalizeNameKey(entry.name))
    );
    ...
    let insertedEntries = existingCodex.entries.slice(0, 0);
    if (rowsToInsert.length > 0) {
      ...
      if (insertError) {
        console.error("Codex entry insert error:", insertError);
        return NextResponse.json(
          { error: "Failed to save generated codex entries" },
          { status: 500 }
        );
      }

      insertedEntries = ((insertedRows ?? []) as DbGeneratedEntryRow[]).map((row) =>
        mapCodexEntryRow(row, [])
      );
    }

    const nameToId = new Map<string, string>();
    for (const entry of [...existingCodex.entries, ...insertedEntries]) {
      nameToId.set(normalizeNameKey(entry.name), entry.id);
    }

    const relationshipKeys = new Set(
      existingCodex.relationships.map(
        (relationship) =>
    ...
    let insertedRelationships = existingCodex.relationships.slice(0, 0);
    if (relationshipRows.length > 0) {
      ...
      if (insertError) {
        console.error("Codex relationship insert error:", insertError);
        return NextResponse.json(
          { error: "Failed to save generated codex relationships" },
          { status: 500 }
        );
      }
    ...
    const message =
      err instanceof Error ? err.message : "Codex generation failed";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}

function parseGeneratedEntries(value: unknown): CodexGenerateEntryInput[] {
  if (!Array.isArray(value)) return [];

  const entries: CodexGenerateEntryInput[] = [];
  for (const item of value) {
    ...
    entries.push({
      name,
      type: type as CodexGenerateEntryInput["type"],
      description,
...
function parseGeneratedRelationships(
  ...
  const relationships: CodexGenerateRelationshipInput[] = [];
  for (const item of value) {
    if (!item || typeof item !== "object" || Array.isArray(item)) continue;
