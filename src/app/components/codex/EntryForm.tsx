...
interface EntryFormProps {
  ...
  saving?: boolean;
  submitLabel: string;
  onSubmit: (input: CreateCodexEntryInput | UpdateCodexEntryInput) => Promise<void>;
  onCancel?: () => void;
...
export default function EntryForm({
  ...
  const [customFields, setCustomFields] = useState<CodexCustomField[]>(
...
function sanitizeFields(fields: CodexCustomField[]): CodexCustomField[] {
  return fields
    .map((field) => ({
      key: field.key.trim(),
...
function getInitialFormState(entry?: CodexEntry | null) {
  return {
    name: entry?.name ?? "",
