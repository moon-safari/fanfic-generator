import { resolveCodexEntryAtChapter } from "../prompts/codex";
import type { CodexEntry } from "../../types/codex";

export interface DetectedCodexMention {
  entryId: string;
  matchedText: string;
  matchedAlias?: string;
  startIndex: number;
...
export function detectCodexMentions(
  chapterContent: string,
  entries: CodexEntry[],
  chapterNumber: number
): DetectedCodexMention[] {
  ...
  const mentions: DetectedCodexMention[] = [];
...
function buildMentionCandidates(
  ...
  for (const entry of entries) {
    const resolved = resolveCodexEntryAtChapter(entry, chapterNumber);
