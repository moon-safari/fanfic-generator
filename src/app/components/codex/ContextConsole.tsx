...
import type { CodexContextMode, StoryContextReasonKey } from "../../types/codex";
...
export default function ContextConsole({
  ...
  const hasNoIncludedCodexEntries =
      ...
      <div className="space-y-4">
        ...
        <section className="rounded-3xl border border-zinc-800 bg-zinc-950/80 p-4">
          ...
          <p className="mt-3 text-xs text-zinc-500">
            ...
            the rest of the included Codex truth. Current active relationships:{" "}
          ...
          {context.source === "story_bible" && (
            <div className="mt-4 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-3 py-2.5 text-sm text-amber-100">
              Codex is empty for this story, so the system is falling back to the
              legacy Story Bible.
            </div>
          )}

          {hasNoIncludedCodexEntries && (
            <div className="mt-4 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-3 py-2.5 text-sm text-amber-100">
              Every Codex entry is currently excluded from prompt context. Future
              writing can still use chapter summaries and manuscript text, but no
              Codex entries are being injected until you reset at least one entry.
            </div>
          )}
        ...
        {context.source === "codex" && context.includedEntryCount > 0 && (
          <section className="rounded-3xl border border-zinc-800 bg-zinc-950/80 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
            ...
            {context.focusEntries.length > 0 ? (
              ...
              <div className="mt-4 rounded-2xl border border-dashed border-zinc-800 px-3 py-3 text-sm text-zinc-500">
                ...
                the included Codex in its normal resolved order.
              </div>
            )}
          </section>
        )}
...
function formatSourceLabel(source: "codex" | "story_bible" | "none") {
  switch (source) {
    case "codex":
      return "Codex";
    case "story_bible":
...
function formatContextModeLabel(mode: CodexContextMode) {
  switch (mode) {
    case "pin":
      return "Pinned";
    case "exclude":
...
function getContextTone(mode: CodexContextMode) {
  switch (mode) {
    case "pin":
...
function describeNextChapterBehavior(
  ...
  if (contextMode === "exclude") {
    return "Excluded entries stay in the Codex but are omitted from the active prompt context.";
  }
...
function buildContextModeNotice(
  ...
  switch (mode) {
    ...
    case "exclude":
      return {
        tone: "warning",
        message: `${entryName} is excluded from active prompt context. It stays in the Codex, but future writing will stop injecting it until you reset it.`,
      };
    default:
      return {
        tone: "success",
        message: `${entryName} is back on automatic context handling. Future writing will include it when the resolved Codex says it is relevant.`,
      };
  }
}
