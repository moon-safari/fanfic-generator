...
interface SuggestionListProps {
  ...
  onAccept: (suggestionId: string) => Promise<MemoryChangeSuggestion>;
  onReject: (suggestionId: string) => Promise<MemoryChangeSuggestion>;
...
function ProgressionPreview({
  ...
  const before = resolveMemoryEntryAtChapter(
    entry,
...
function ResolvedEntryPreview({ entry }: { entry: ResolvedMemoryEntry }) {
...
function getSuggestionOpenEntryId(suggestion: MemoryChangeSuggestion): string | null {
...
function getRoleLabel(entry?: MemoryEntry): string | undefined {
  if (!entry) {
    return undefined;
  }
...
function labelStatus(status: MemoryChangeSuggestion["status"]): string {
  switch (status) {
    case "accepted":
      return "Accepted";
    case "applied":
...
function labelChangeType(changeType: MemoryChangeSuggestion["changeType"]): string {
  switch (changeType) {
    case "create_entry":
      return "New fact";
    case "update_entry_aliases":
...
function getStatusClasses(status: MemoryChangeSuggestion["status"]): string {
  switch (status) {
    case "accepted":
    case "applied":
      return "bg-emerald-500/15 text-emerald-200";
...
function getConfidenceClasses(
  confidence: MemoryChangeSuggestion["confidence"]
): string {
  switch (confidence) {
    case "high":
      return "bg-emerald-500/15 text-emerald-200";
